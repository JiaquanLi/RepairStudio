...
CColorBox::CColorBox()
{
	m_color = RGB(250,250,250);
	m_UnCheckcolor = RGB(250,250,250);
...
void CColorBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	else
	{
		...
		{
			dc.FillSolidRect(&rect,	m_UnCheckcolor);
		}
	}
...
void CColorBox::SetUnCheckColor(const COLORREF newColor)
{
	m_UnCheckcolor=newColor;
	Redraw();

}
COLORREF CColorBox::GetUnCheckColor() const
{
	return m_UnCheckcolor;
}
