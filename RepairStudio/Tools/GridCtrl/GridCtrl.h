...
public:
	...
	void SetImageMid(BOOL bMidX, BOOL bMidY) {
		m_bImageMidX = bMidX; 
		m_bImageMidY = bMidY;
	}

	void GetImageMid(BOOL& bMidX, BOOL& bMidY) {
		bMidX	= m_bImageMidX;
		bMidY = m_bImageMidY;
	}
...
public:
	...
	BOOL   SetItemDoubleData(int nRow, int nCol, double dbData,int nBitNum = 3);
	BOOL   SetItemIntData(int nRow, int nCol, int nData,int nBitNum = 0);
	BOOL SetItemCurSel(int nRow, int nCol, int nIndex, CStringArray& strTemp);
	...
	double GetItemDoubleData(int nRow, int nCol);
	int GetItemIntData(int nRow, int nCol);
	int GetItemCurSel(int nRow, int nCol,CStringArray& strTemp);
	...
	//设置grid的风格特征
	void fnSetGridStyle(BOOL bRow = TRUE);
...
protected:
	...
	//??¨°¨??¨a?¤2????á?¨o?D?
	CString m_strRight,m_strCenter,m_strLeft;
