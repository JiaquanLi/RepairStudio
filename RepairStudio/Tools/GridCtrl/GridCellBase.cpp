 ...
 //           rect.right--; rect.bottom--;
    ...
    if (pGrid->GetImageList() && GetImage() >= 0)
    {
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
			...
			if (bMidX )
			{
				nOffsetX = (rect.right  - rect.left - nImageWidth) / 2;
			}
			if(bMidY)
			{
				nOffsetY = (rect.bottom - rect.top - nImageHeight) / 2;
			}

			point = rect.TopLeft() + CPoint(nOffsetX, nOffsetY);
				...
				&& nImageHeight + rect.top <= rect.bottom + (int)(2 * GetMargin()))
			{
				...
				pGrid->GetImageList()->Draw(pDC, GetImage(), point, ILD_NORMAL);
			}
 ...
 //   DrawText(pDC->m_hDC, GetText(), -1, rect, GetFormat() | DT_NOPREFIX);
	...
	else 
	{
		...
		for (int i = 0; i < tokenCount; i++)
		{
			...
			DrawText(pDC->m_hDC, strAry.GetAt(i), -1, tempRect, GetFormat() |
				DT_NOPREFIX);
		}
	}
	strAry.RemoveAll();
