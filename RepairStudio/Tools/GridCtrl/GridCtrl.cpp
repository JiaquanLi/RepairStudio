...
BOOL  CGridCtrl::SetItemDoubleData(int nRow, int nCol, double dbData,int nBitNum)
{
	CString strText;
	...
	if (1 == nBitNum)
	{
		strText.Format("%.1f",dbData);
	}
	else if (2 == nBitNum)
	{
		strText.Format("%.2f",dbData);
	}
	else if (4 == nBitNum)
	{
		strText.Format("%.4f",dbData);
	}
	else
	{
		strText.Format("%.3f",dbData);
	}

	return SetItemText(nRow,nCol,strText);
}

BOOL  CGridCtrl::SetItemIntData(int nRow, int nCol, int nData,int nBitNum)
{
	CString strText;
	...
	if (1 == nBitNum)
	{
		strText.Format("%01d",nData);
	}
	else if (2 == nBitNum)
	{
		strText.Format("%02d",nData);
	}
	else if (3 == nBitNum)
	{
		strText.Format("%03d",nData);
	}
	else if (4 == nBitNum)
	{
		strText.Format("%04d",nData);
	}
	else
	{
...
BOOL CGridCtrl::SetItemCurSel(int nRow, int nCol, int nIndex, CStringArray& strTemp)
{
	if (nIndex >= strTemp.GetSize())
	{
		return FALSE;
	}

	return SetItemText(nRow, nCol, strTemp.GetAt(nIndex));
}
...
//设置grid的风格特征
 void CGridCtrl::fnSetGridStyle(BOOL bRow)
{
	 //禁止拉伸
	 SetRowResize(FALSE);
	...
	//设置每个单元格的类型
	 SetFixedBkColor(COLOR_GRID_HEAD);

	 int nFixRow = GetFixedRowCount();
	 int nFixCol = GetFixedColumnCount();

	for (int i = nFixRow; i < m_nRows; i++)
	{
		for (int j = nFixCol; j < m_nCols; j++)
		{
			//全部居中
			SetItemFormat(i, j, GetItemFormat(i, j) | DT_CENTER);

			if (bRow)
			{
				if (0 == (i % 2))
				{
					SetItemBkColour(i, j, COLOR_GRID_ROW1);
				}
				else
				{
					SetItemBkColour(i, j, COLOR_GRID_ROW2);
				}
			}
			else
			{
...
void CGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
	...
	//Used for merge cells增加该代码，则最后一条线打印不出来
...
double CGridCtrl::GetItemDoubleData(int nRow, int nCol)
{
...
int CGridCtrl::GetItemIntData(int nRow, int nCol)
{
	CString strData = GetItemText(nRow, nCol);
...
int CGridCtrl::GetItemCurSel(int nRow, int nCol, CStringArray& strTemp)
{
	int nIndex = -1;
