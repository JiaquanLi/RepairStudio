...
public:
	...
	bool Open(DWORD dwPort, LPCTSTR szPortName, DWORD dwBaudRate, BYTE btParity = NOPARITY, BYTE btByteSize = 8, BYTE btStopBits = ONESTOPBIT)
	{
		...
		_tcscpy_s(szName_, sizeof(szPortName),szPortName);
	...
	DWORD ReadString(char *szBuffer, DWORD dwLength, DWORD dwWaitTime = INFINITE)
	{
		CN_ASSERT(szBuffer);
		DWORD dsSize = Read(szBuffer, dwLength - 1, dwWaitTime);
		return dsSize;
	}
	...
	DWORD Write(wchar_t *szBuffer, wchar_t * szFormat, ...)
	{
		va_list va;
		va_start(va, szFormat);
		vswprintf(szBuffer,512,szFormat, va);
		va_end(va);
	...
	static bool Assert(LPCTSTR szMessage)
	{
		...
		_tcscpy_s(szMsg,sizeof(szMessage),szMessage);
